...
postulate
  ...
  abs♭ : {Φ : CtxVar} → AbsTy Φ → AbsTy Φ
  --♯♯ : ∀{Φ} → {T : AbsTy Φ} → ♯ ♯ T ≡ ♯ T
  --♯♭ : ∀{Φ} → {T : AbsTy Φ} → ♯ ♭ T ≡ ♯ T
  --♭♯ : ∀{Φ} → {T : AbsTy Φ} → ♭ ♯ T ≡ ♭ T
  abs♭♭ : ∀{Φ} → {T : AbsTy Φ} → abs♭ (abs♭ T) ≡ abs♭ T
--{-# REWRITE ♯♯ #-}
--{-# REWRITE ♯♭ #-}
--{-# REWRITE ♭♯ #-}
{-# REWRITE abs♭♭ #-}
...
postulate
  ...
  absκ : ∀{Φ} → {T : AbsTy Φ} → AbsTm Φ (abs♭ T) → AbsTm Φ T
  --ι♯ : ∀{Φ} → {T : AbsTy Φ} → {t : AbsTm Φ (♯ T)} → ι t ≡ t
  --κ♯ : ∀{Φ} → {T : AbsTy Φ} → {t : AbsTm Φ (♭ ♯ T)} → κ t ≡ ι t
  κ♭ : ∀{Φ} → {T : AbsTy Φ} → {t : AbsTm Φ (abs♭ (abs♭ T))} → absκ t ≡ t
--{-# REWRITE ι♯ #-}
--{-# REWRITE κ♯ #-}
{-# REWRITE κ♭ #-}
...
cκ : ∀{Γ} → Sub (c♭ Γ) Γ
c♭ • = •
c♭ (Γ „ x ∈ T) = c♭ Γ „ x ∈ (λ Φ γ → abs♭ (T Φ (cκ Φ γ)))
cκ {•} _ _ = •
cκ {Γ „ .x ∈ T} Φ (γ “ _ ∋ t / x) = (cκ Φ γ) “ T ∋ absκ t / x
